...
grid_search <- function(data, max_pq_farima = 3, max_pq_garch = 3) {
  best_aic <- Inf  # Initialiser le meilleur AIC
  best_model <- NULL
  best_params <- list()

  # Boucle pour p et q de FARIMA
  for (p_farima in 0:max_pq_farima) {
    for (q_farima in 0:max_pq_farima) {
      
      # Boucle pour p et q de GARCH
      for (p_garch in 0:max_pq_garch) {
        for (q_garch in 0:max_pq_garch) {
  ...
  return(list(best_model = best_model, best_params = best_params, best_aic = best_aic))
}
...
```{r}
# Afficher le résumé du modèle optimal
best_model
...
residuals <- residuals(best_model, standardize = TRUE)
...
spec <- ugarchspec(
  ...
  mean.model = list(armaOrder = c(1, 1), include.mean = TRUE, archm = FALSE, arfima = TRUE),  # ARFIMA(1,d,1)
  distribution.model = "norm"  # Distribution normale
)

# Ajuster le modèle sur les données
fit <- ugarchfit(spec = spec, data = na.omit(Rend))
