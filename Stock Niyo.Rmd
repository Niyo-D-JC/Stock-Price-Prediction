...
train_end_date <- as.Date("2024-01-30")
...
variable_contribution <- function(model, data, test_data, train_end_date) {
  ...
  for (var in colnames(data)[3:ncol(data)]) {  # Ignorer ds et y
    ...
    # Contribution = Différence de RMSE
    contributions[[var]] <- permuted_rmse - base_rmse
  }
...
plot_contributions <- function(contributions_df) {
  ggplot(contributions_df[4:30,], aes(x = reorder(Variable, Contribution), y = Contribution, fill = Contribution)) +
